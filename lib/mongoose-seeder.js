        ...
        _seed: function(data, done) {
            ...
            async.eachSeries(Object.keys(data), function(key, next) {
                ...
                try {
                    ...
                    async.series([
                        function(callback) {
                            if(_this.options.dropCollections === true && drop !== false) {
                                // Drop the collection
                                mongoose.connection.db.dropCollection(Model.collection.name, function(err) {
                                    if(err) {
                                        // Do not continue on this collection because something happened when tried to drop the collection
                                        return callback(err);
                                    }
                        ...
                        if(err) {
                            // If an error occurs, do not stop the execution but push it on the errors stack
                            errors.push(err);
                        }
                ...
                if(errors.length === 0) {
                    // Make sure to pass in undefined as error parameter if we don't have errors.
                    errors = undefined;
                }

                done(errors, _this.result);
            });
        },
        /**
